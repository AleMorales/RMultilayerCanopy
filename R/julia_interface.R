...
#' excess energy as potential assimilation (\code{"E}) and the relative excess energy
#' (\code{"rE}).
#' @export
#'
#' @examples
...
A = function(pars = parameters(), Np = 0.1, f_Nc = 0.15, f_Nr = 0.30,
  ...
  out = JuliaCall::julia_call("MC.Ag", pars, Np, f_Nc, f_Nr, VPD*1e3,
                              ...
                              need_return = "R")
  c(Ag = out$Ag*1e6, A = out$A*1e6, Ac = out$Ac*1e6,
    Aj = out$Aj*1e6, E = out$E*1e6, rE = out$rE)
}
...
#' \item{Ashade}{Average shade gross CO2 assimilation per unit of shaded leaf area in each layer (mumol/m2/s)}
#' \item{Asun}{Average sunlit gross CO2 assimilation per unit of sunlit leaf area in each layer (mumol/m2/s)}
#' }
#' @export
#'
#' @examples
...
#' List with the following information:
#' \describe{
#' \item{Ican}{Total PAR intercepted by canopy per unit of ground (mumol/m2/s)}
#' \item{Ilayer}{PAR intercepted by each layer per unit of ground (mumol/m2/s)}
#' }
#' @export
#'
#' @examples
...
Ican = function(can, env, pars, vars) {
  JuliaCall::julia_library("MultilayerCanopy")
  JuliaCall::julia_eval("const MC = MultilayerCanopy")
  out = JuliaCall::julia_call("MC.Ican", can, env, pars, vars, need_return = "R")
  out$Ican   = out$Ican*1e6
...
Ecan = function(can, env, pars, vars) {
  JuliaCall::julia_library("MultilayerCanopy")
  JuliaCall::julia_eval("const MC = MultilayerCanopy")
  out = JuliaCall::julia_call("MC.Ecan", can, env, pars, vars, need_return = "R")
  out$Ecan   = out$Ecan*1e6
  out$Elayer = out$Elayer*1e6
  out$Eshade = out$Eshade*1e6
  out$Esun   = out$Esun*1e6
  out
}
...
Adiurnal = function(weather, can, pars, vars, nsteps = 3, ngrid = 5) {
  JuliaCall::julia_library("MultilayerCanopy")
  JuliaCall::julia_eval("const MC = MultilayerCanopy")
  ...
  JuliaCall::julia_call("MC.Adiurnal", weather, can, pars, vars, method,
                        need_return = "R")
}
...
Idiurnal = function(weather, can, pars, vars, nsteps = 3, ngrid = 5) {
  JuliaCall::julia_library("MultilayerCanopy")
  JuliaCall::julia_eval("const MC = MultilayerCanopy")
  ngrid = JuliaCall::julia_call("Val", as.integer(ngrid))
  nsteps = JuliaCall::julia_call("Val", as.integer(nsteps))
  method = JuliaCall::julia_call("MC.PGLIntegrator", ngrid, nsteps)
  JuliaCall::julia_call("MC.Idiurnal", weather, can, pars, vars, method,
                        need_return = "R")
}
...
Ediurnal = function(weather, can, pars, vars, nsteps = 3, ngrid = 5) {
  JuliaCall::julia_library("MultilayerCanopy")
  JuliaCall::julia_eval("const MC = MultilayerCanopy")
  ngrid = JuliaCall::julia_call("Val", as.integer(ngrid))
  nsteps = JuliaCall::julia_call("Val", as.integer(nsteps))
  method = JuliaCall::julia_call("MC.PGLIntegrator", ngrid, nsteps)
  JuliaCall::julia_call("MC.Ediurnal", weather, can, pars, vars, method,
                        need_return = "R")
}
